...
#include "thirdparty/vector/vector.hpp" // sjtu::vector
...
namespace RFlowey {
  ...
  public:
    ...
    if(manager_.is_new) {
      ...
      PagePtr<BPT_config> cfg_ptr{DISK_PAGE_CONFIG_ID, &manager_}; // Ensure DISK_PAGE_CONFIG_ID is defined
    ...
    ~BPT() {
      ...
      if (root_.page_id() != INVALID_PAGE_ID && root_.page_id() != 0) {
        ...
        PagePtr<BPT_config>{DISK_PAGE_CONFIG_ID, &manager_}.make_ref(std::move(cfg_to_save));
      }
    }
    ...
    sjtu::vector<pair<Key,Value>> range_find(const Key& start_key, const Key& end_key) {
        ...
        // Optional: Check if start_key > end_key if Key supports it meaningfully.
        // if (start_key > end_key) { return result_values; } // Assuming Key has operator>
        ...
        if (!current_leaf.is_valid) return result_values; // Tree is empty or error

        // Adjust current_idx to the first actual element >= start_key
        if (current_idx == INVALID_PAGE_ID) {
            // start_key is smaller than all keys in the node found (or node is empty but find_pos should handle this)
            // or smaller than the first key in the leaf.
            current_idx = 0; // Start scan from index 0 of this leaf.
        }

        // current_idx from search points to the largest key <= start_key.
        // We need to find the first key that is >= start_key.
        while (current_idx < current_leaf->current_size_ && current_leaf->at(current_idx).first < start_key) {
            current_idx++;
        }
        // Now current_idx points to the first key >= start_key, or current_leaf->current_size_
        ...
        while (current_leaf.is_valid) {
            for (; current_idx < current_leaf->current_size_; ++current_idx) {
                ...
                // Check against end_key (inclusive)
                if (item_key > end_key) { // Current item's key is past the end_key
                    return result_values; // All subsequent keys will also be > end_key.
                }
